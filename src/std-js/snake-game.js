...
var $body = $("body"),
    ...
    canvasWidth,
    canvasHeight,
    canvasVerticalBlockSize,
    canvasHorizontalBlockSize,
    canvasMiddleVer,
    canvasMiddleHor,
    freeIndexes = [],
    players = {
...
var _loop = function _loop(i) {
  $("div#" + directions[i] + "Button").on("click", function () {
    if ($.inArray(directions[i], allowedDirections[players.users[0].direction]) !== -1) players.users[0].direction = directions[i];
  });
};
...
$(document).ready(function () {
  ...
  canvasWidth = bodyWidth - bodyWidth % snakeSize, canvasHeight = bodyHeight - bodyHeight % snakeSize,
  /*
  ...
  canvasVerticalBlockSize = canvasHeight / snakeSize; // Set all indexes as free

  for (var i = 1; i <= canvasHorizontalBlockSize; i++) {
    freeIndexes[i] = [];

    for (var j = 1; j <= canvasVerticalBlockSize; j++) {
...
function play() {
  ...
  players.bots[6].y = Math.floor(canvasVerticalBlockSize / 8 * 7); // Move all players, including users and bots
...
function movePlayer(player) {
  var alBot = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  setTimeout(function () {
    if (alBot) checkAvailableWays(player);else switch (player.direction) {
      case "right":
        player.x++;
        break;
    ...
    if (isIndexFree(x, y) && !gameOver) {
      ...
      if (lostPlayersCount + 1 === players.count || players.user.lost) {
        gameOver = true;
        if (!players.user.lost) alert("You won!");else alert("You lost!");
      }
    }
  }, 50);
...
function checkAvailableWays(alBot) {
  var checkAllowed = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  var moveInfo = [["right", "x", 1], ["left", "x", -1], ["up", "y", -1], ["down", "y", 1]],
      randomMove = {};
  ...
  if (randomInt(0, 200) === 0) alBot.direction = allowedDirections[alBot.direction][randomInt(0, 1)];

  if (checkAllowed) {
    var i = randomMove.start;
    ...
    for (var _i = 0; _i < 4; _i++) {
      if (moveInfo[_i][0] === alBot.direction) {
        var _axises = {
          x: alBot.x,
          y: alBot.y
        },
            _move = moveInfo[_i];
        _axises[_move[1]] += _move[2];

        if (isIndexFree(_axises.x, _axises.y)) {
          alBot[_move[1]] += _move[2];
          return true;
        } else return checkAvailableWays(alBot, true);
      }
    }
  }
}
...
function isIndexFree(x, y) {
  if (typeof freeIndexes[x] === "undefined") return false;
  if (typeof freeIndexes[x][y] === "undefined") return false;
